...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected final void onRestoreInstanceState(Parcelable state) {
		...
		//保证不抛出
		// IllegalArgumentException: Wrong state class, expecting View State but received class android.support.v7.widget.RecyclerView$SavedState instead. This usually happens when two views of different type have the same id in the same hierarchy. This view's id is id/pullToRefreshRecyclerView. Make sure other views do not use the same id.
		//at andro
		try {
